...
import { formatTimestamp } from "../../utils/helperFuncrion";
...
const getFirstSeen = async () => {
  try {
    ...
    const firstSeen =
      (await AsyncStorage.getItem(Constant.USER_FIRST_SEEN)) || "";
    return firstSeen;
  } catch (error) {
...
const saveFirstSeen = async () => {
  ...
  try {
    ...
    if (!storedFirstSeen) {
      ...
      // Use AsyncStorage to store the first seen timestamp asynchronously
...
const getCookieId = async () => {
  try {
    const storedCookieId =
      (await AsyncStorage.getItem(Constant.COOKIE_ID)) || "";
...
export { retrieveData, saveCookieId, saveFirstSeen, storeData };
